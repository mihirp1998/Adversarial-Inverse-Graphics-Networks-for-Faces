...
with open("mouth_bb.pickle", "rb") as fp:
...
train_writer = tf.summary.FileWriter('./logs_ip_lips3/train')
test_writer = tf.summary.FileWriter('./logs_ip_lips3/test')
...
blips_data_dir = '/home/wseto/datasets/celeba_blips'
slips_data_dir = '/home/wseto/datasets/celeba_slips'

blips_data = glob(os.path.join(blips_data_dir, "*.png"))
slips_data = glob(os.path.join(slips_data_dir, "*.png"))

data_disc, blips_data_nondisc = train_test_split(blips_data, test_size=0.1, random_state=42)

blips_data_train, blips_data_sample = train_test_split(blips_data_nondisc, test_size=0.5, random_state=42)
slips_data_train, slips_data_sample = train_test_split(slips_data, test_size=0.08, random_state=42)

data_train = blips_data_train + slips_data_train
data_sample = blips_data_sample + slips_data_sample
