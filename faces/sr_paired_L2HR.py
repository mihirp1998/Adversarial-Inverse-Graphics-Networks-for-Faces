...
# experiment: paired GAN with L2 loss on the high resolution images
...
def create_discriminator(inputs, b_reuse=False):
    with slim.arg_scope([slim.conv2d],
        ...
        disc = lrelu(slim.conv2d(inputs, 64, [5, 5], scope='conv1'))
        print_shape(disc)
...
g_loss = gen_L2_HR + 0.001*g_loss_adv
...
train_writer = tf.summary.FileWriter('./logs_p_HR4/train')
test_writer = tf.summary.FileWriter('./logs_p_HR4/test')
...
samples_dir = 'samples_p_HR4'
if not os.path.exists(samples_dir):
    os.makedirs(samples_dir)

checkpoint_dir = 'checkpoint_p_HR4'
...
b_load = False
ckpt_dir = '/home/wseto/dcgan/checkpoint_reg_pt/model-60002'
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        for idx in xrange(num_batches):
            ...
            fetches = [d_loss_fake, d_loss_real, g_loss, d_optim, g_optim]
            errD_fake, errD_real, errG, _, _ = sess.run(fetches, feed_dict={ inputs: batch_inputs, real_ims: batch_origs})
